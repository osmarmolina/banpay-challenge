...
export class AuthService extends PrismaClient implements OnModuleInit {
    ...
    async register(registerDto: RegisterDto) {
        try {
            ...
            const user = await this.findeOneUser({ email: registerDto.email, username: registerDto.username })
            if (user) {
                throw new RpcException({ status: HttpStatus.BAD_REQUEST, message: 'Username or email is already taken' })
            }
            ...
            throw new RpcException({ status: error.status ?? 400, message: error.message ?? 'An error occurred' })
        }
    ...
    async login(loginDto: LoginDto) {
        try {
            ...
            if (!user) {
                throw new RpcException({ status: HttpStatus.UNAUTHORIZED, message: 'Email or password is incorrect' })
            }
            ...
            const response = {
                ...
                token: await this.signJwt({ id: user.id, username: user.username, email: user.email, role: user.role })
            }

            return response

        } catch (error) {
            throw new RpcException({ status: error.error.status ?? 400, message: error.message ?? 'An error occurred' })
        }
    }
